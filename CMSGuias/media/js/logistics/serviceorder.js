...
var addItem, calcamount, changeDsct, changeRadio, changeSelIGV, getItemizerByProject, getListTmp, listDetails, loadEdit, saveServiceOrder, selectDel, selectProject, showList, showNew;
...
saveServiceOrder = function(event) {
  ...
  data.itemizer = $("#itemizer").val() || '';
  ref = Object.keys(data);
  for (i = 0, len = ref.length; i < len; i++) {
    x = ref[i];
    if (data[x] === "" && (x !== 'quotation' && x !== 'project' && x !== 'itemizer' && x !== 'subproject')) {
      valid = false;
      break;
    } else {
      valid = true;
