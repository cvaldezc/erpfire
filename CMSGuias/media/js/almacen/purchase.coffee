  ...
  if data.type is "code"
    ...
    if data.start.length is 10
      ...
      $().toastmessage "showWarningToast",
        "No se han ingresado la fecha a buscar."
  else
    data.pass = true
  if data.pass
    $.getJSON "", data, (response) ->
      if response.status
        ...
        $().toastmessage "showWarningToast",
          "Se han encontrado errores. #{response.raise}"
        return
  return
...
showIngressInventory = (event) ->
  ...
  $.getJSON "", "purchase" : btn, (response) ->
    if response.status
      ...
      # $(".deposit").append "<a target="_blank"
      # class="btn btn-warning btn-xs text-black\"
      # href=\"/media/#{response.head.deposit}\"><span
      #  class=\"glyphicon glyphicon-cloud-download\"></span></a>"
      count = 1
      response['read'] = -> if this.dunit is this.unit then true else if this.dunit is '' then true else false
...
loadIngress = (event) ->
  ...
  if arr.length
    ...
    $().toastmessage "showWarningToast",
      "Seleccione por lo menos un material para hacer el ingreso a almacén."
  return
...
saveNoteIngress = (response) ->
  ...
  valfield = {'invoice': 'Factura', 'guide': 'Guia Remisión'}
  $(".mingress > div > div > div.modal-body > div.row")
  .find("input, select").each (index, element) ->
    ...
    if element.value isnt ""
      ...
      $().toastmessage "showWarningToast",
        "Campo vacio, #{valfield[element.name]}"
      pass = false
      return pass
        ...
        if result is "Si"
          ...
          $.post "", data, (response) ->
            if response.status
              ...
              $(".show-note-ingress").attr "href",
                "/reports/note/ingress/#{response.ingress}/"
              return
            else
              $().toastmessage "showWarningToast",
                "No se a podido generar la Nota de Ingreso. #{response.raise}"
              return
          return
  return