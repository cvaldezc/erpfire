...
from CMSGuias.apps.home.models import Employee, Moneda, Configuracion
...
from CMSGuias.apps.ventas.models import Sectore
from .models import *
            ...
            if request.is_ajax():
                try:
                    ...
                    if 'saveherramienta' in request.POST:
                        ...
                        Materiale.objects.create(
                            materiales_id=codherra,
                            matnom=request.POST.get('nameherra'),
                            matmed=request.POST.get('medherra'),
                            unidad_id=request.POST.get('unidherra'),
                            matacb='Fabrica',
                            matare=0,
            ...
            try:
                if 'lherraalmacen' in request.GET:
                    ...
                    for x in detGuiaHerramienta.objects.filter(
                        ...
                        guia__estado = 'GE').distinct('herramienta__matnom','herramienta__matmed').order_by('herramienta__matmed'):
                        lestalmacen.append(
                            {'codherra':x.herramienta_id,
                            'nameherra':x.herramienta.matnom,
                            'medherra':x.herramienta.matmed,
...
class Cargar(JSONResponseMixin, TemplateView):
    ...
    def get(self,request,*args,**kwargs):
        if 'descformat' in request.GET:
            file_path = "/home/juan/Desktop/formato_ingreso.xls"
            with open(file_path, 'rb') as fh:
                response = HttpResponse(fh.read(), content_type='application/vnd.ms-excel;charset=utf-16')
                response['Content-Disposition'] = 'inline; filename=formato_ingreso.xls'
            return response
